...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    // If rejecting, refund user balance (assuming it was deducted on request)
    if ($status == 'Rejected') {
        $wit_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `total-withdraw` WHERE id = $id"));
        if ($wit_data && $wit_data['status'] == 'Pending') {
    ...
    mysqli_query($conn, "UPDATE `total-withdraw` SET status = '$status' WHERE id = $id");
    header("Location: manage-withdrawals.php");
    exit();
}

$withdrawals = mysqli_query($conn, "SELECT w.*, u.name, u.email FROM `total-withdraw` w JOIN users u ON w.uid = u.id ORDER BY w.id DESC");
...
<body class="flex min-h-screen">
    ...
    <div class="sidebar w-64 text-white flex flex-col fixed h-full shadow-2xl">
        ...
        <nav class="flex-1 mt-6">
            ...
            <a href="manage-withdrawals.php" class="nav-link active flex items-center gap-4 px-6 py-4 transition">
    ...
    <div class="flex-1 ml-64 p-10">
        <header class="flex justify-between items-center mb-10">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Withdrawal Requests</h1>
                <p class="text-gray-500">Review and process user cashouts</p>
            </div>
